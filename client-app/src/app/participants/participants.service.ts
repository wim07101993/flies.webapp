...
export class ParticipantsService {
  ...
  updateName(id: number, newName: string) : Observable<ParticipantDTO> {
    return this.http.put<ParticipantDTO>(`${this.apiUrl}/participants/${id}/name/?name=${newName}`, "");
  }

  updateScore(id: number, score: number) : Observable<ParticipantDTO> {
    return this.http.put<ParticipantDTO>(`${this.apiUrl}/participants/${id}/score/?score=${score}`, "");
  }

  deleteParticipant(id: number) {
    return this.http.delete<ParticipantDTO>(`${this.apiUrl}/participants/${id}`)
  }
}
